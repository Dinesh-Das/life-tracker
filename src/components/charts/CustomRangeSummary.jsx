...
export default function CustomRangeSummary({ data = [], year }) {
    ...
    return <section className="glass-card" style={{ padding: '20px', marginBottom: 20 }}>
        <div className="mobile-stack" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
            ...
            <div className="analytics-range-controls"><select aria-label="Analytics range preset" value={preset} onChange={event => selectPreset(event.target.value)}><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option><option value="custom">Custom</option></select><input aria-label="Analytics start date" type="date" value={range.from} onChange={event => { setPreset('custom'); setRange({ ...range, from: event.target.value }); }} /><input aria-label="Analytics end date" type="date" value={range.until} onChange={event => { setPreset('custom'); setRange({ ...range, until: event.target.value }); }} /></div>
        </div>
